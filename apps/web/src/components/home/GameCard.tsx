...
const CATEGORY = {
  ...
  instant: { label: "Instant", cls: "bg-emerald-500/90 text-white" },
  sport:   { label: "Esportes",cls: "bg-orange-500/90 text-white" },
} as const;

// Gradient used as thumbnail placeholder per category
const THUMB_GRADIENT: Record<GameCardData["category"], string> = {
...
export function GameCard({ game, index = 0, className = "" }: GameCardProps) {
  const prefersReduced = useReducedMotion();
  const [imgLoaded, setImgLoaded] = useState(false);
  const cfg = CATEGORY[game.category];
    ...
    >
      ...
      >
        ...
        <div className="relative aspect-[4/3] overflow-hidden">
          ...
          {/* Real thumbnail — hidden until loaded */}
          {game.thumbnailUrl && (
            <>
              {!imgLoaded && (
